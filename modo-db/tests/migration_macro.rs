use modo_db::MigrationRegistration;

#[modo_db::migration(version = 9000, description = "Test migration")]
async fn test_migration(db: &sea_orm::DatabaseConnection) -> Result<(), modo::Error> {
    let _ = db;
    Ok(())
}

#[modo_db::migration(version = 9001, description = "Another test migration")]
async fn another_test(db: &sea_orm::DatabaseConnection) -> Result<(), modo::Error> {
    let _ = db;
    Ok(())
}
