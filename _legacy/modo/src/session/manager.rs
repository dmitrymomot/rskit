...
/// ```rust,ignore
/// #[handler(POST, "/login")]
/// async fn login(session: SessionManager, db: Db) -> Result<Redirect, Error> {
...
impl SessionManager {
    ...
    pub async fn authenticate(&mut self, user_id: &str) -> Result<(), Error> {
        ...
        let token = self
            ...
            .current_session
            .as_ref()
            .ok_or_else(|| Error::internal("session not found after create"))?
            .token
            .clone();
    ...
    ///
    /// The session cookie is removed automatically on the response.
    pub async fn logout_all(&mut self) -> Result<(), Error> {
        if let Some(ref session) = self.state.current_session {
            self.state
                .store
    ...
    pub async fn logout_other(&mut self) -> Result<(), Error> {
        let session = self
            .state
            .current_session
            .as_ref()
    ...
    pub async fn set(&mut self, key: &str, value: impl Serialize) -> Result<(), Error> {
        let session = self
            .state
            .current_session
            .as_ref()
    ...
    pub async fn remove_key(&mut self, key: &str) -> Result<(), Error> {
        let session = self
            .state
            .current_session
            .as_ref()
...
mod tests {
    ...
    impl SessionStore for FailingDestroyStore {
        ...
        async fn touch(&self, id: &SessionId, ttl: std::time::Duration) -> Result<(), Error> {
        ...
        async fn destroy(&self, _id: &SessionId) -> Result<(), Error> {
            Err(Error::internal("simulated destroy failure"))
        }
