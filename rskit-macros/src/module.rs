...
pub fn expand(attr: TokenStream, item: TokenStream) -> Result<TokenStream> {
    ...
    let middleware_vec = if wrapper_names.is_empty() {
        ...
        quote! { vec![#(#wrapper_names as rskit::router::RouterMiddlewareFn),*] }
    };
    ...
    Ok(quote! {
        ...
        rskit::inventory::submit! {
            rskit::router::ModuleRegistration {
                name: #mod_name_str,
                prefix: #prefix,
...
/// Rewrite `#[rskit::handler(METHOD, "/path")]` or `#[handler(METHOD, "/path")]`
    ...
    for attr in func.attrs.iter_mut() {
        let is_handler = attr.path().is_ident("handler")
            || (attr.path().segments.len() == 2
                && attr.path().segments[0].ident == "rskit"
