...
- **Template functions** (`tier_name`, `tier_has`, `tier_enabled`, `tier_limit`) are injected by `TemplateContextLayer` when `TierInfo` is in request extensions. No extra setup needed beyond applying `TierLayer`.
