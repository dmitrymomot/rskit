...
- Version sync: `Cargo.toml`, `.claude-plugin/plugin.json`, `.claude-plugin/marketplace.json`, all doc comments (`//!` in `mod.rs`/`lib.rs`), READMEs (`src/**/README.md` and `skills/**/README.md`), skill references (`skills/*/references/*.md`), and root `README.md` must always use the same version. When bumping, also grep for shortened forms (`0.10`, `v0.10`, `modo 0.10`) — not just the full `0.10.4` — since READMEs and skill docs often pin only the major.minor
